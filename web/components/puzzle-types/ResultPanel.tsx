...
function getTimeBracket(
    openedAt: string | null,
    completedAt: string | null
): { base: number; label: string } | null {
    if (!openedAt || !completedAt) return null;
    const diff = Math.max(
        0,
        (new Date(completedAt).getTime() - new Date(openedAt).getTime()) / 1000
    );
...
function formatDuration(
    ...
    const seconds = Math.max(
        0,
        Math.floor((new Date(completedAt).getTime() - new Date(openedAt).getTime()) / 1000)
    );
...
function formatMatchAnswer(
    ...
    try {
        ...
        return answer
            .split(',')
            .map((idx, i) => ({ left: left[i], right: right[Number(idx)] }))
...
function formatConnectionsAnswer(
    answer: string,
    question: string
): { category: string; items: string[] }[] | null {
    try {
        const { items, categories } = JSON.parse(question) as {
            items: string[];
            categories?: string[];
        };
...
export default function ResultPanel({
    ...
    const timeBracket = rawTimeBracket
        ? {
              base: Math.max(0, rawTimeBracket.base),
              label: rawTimeBracket.base <= 0 ? 'solved after 24h+' : rawTimeBracket.label,
          }
        : null;
