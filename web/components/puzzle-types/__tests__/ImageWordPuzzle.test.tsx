...
describe('ImageWordPuzzle', () => {
    it('renders the prompt and image', () => {
        ...
        render(
            <ImageWordPuzzle puzzle={puzzle} solved={false} onSubmit={() => {}} loading={false} />
        );

        expect(screen.getByTestId('puzzle-question')).toHaveTextContent('What is shown?');
        expect(screen.getByRole('img', { name: 'puzzle image' })).toHaveAttribute(
            'src',
            '/test.jpg'
        );
    });
    ...
    it('calls onSubmit with trimmed value', async () => {
        ...
        const puzzle = makePuzzle(VALID_QUESTION);
        render(
            <ImageWordPuzzle puzzle={puzzle} solved={false} onSubmit={onSubmit} loading={false} />
        );
    ...
    it('hides form when solved', () => {
        const puzzle = makePuzzle(VALID_QUESTION);
        render(
            <ImageWordPuzzle puzzle={puzzle} solved={true} onSubmit={() => {}} loading={false} />
        );
