...
interface WheelData { letters: (string | null)[] }
interface WordWheelData { prompt: string; wheels: WheelData[] }
...
function WheelSvg({ letters, size = 170 }: { letters: (string | null)[]; size?: number }) {
  ...
  return (
    <svg width={size} height={size} style={{ display: "block", margin: "0 auto" }}>
      ...
      <circle cx={cx} cy={cy} r={r * 0.6} fill="transparent" stroke="var(--border)" strokeWidth={1.5} />
    </svg>
  );
}
  ...
  return (
    <>
      ...
      {!solved && (
        ...
        >
          <span className="gt">&gt;</span>{loading ? "Checking..." : "Submit Words"}
        </button>
      )}
    </>
  );
