...
interface Props { puzzle: Puzzle; solved: boolean; onSubmit: (g: string) => void; onHint: () => void; loading: boolean; }
interface CountdownData { prompt: string; target: number; numbers: number[]; operators: string[]; }
...
export default function CountdownPuzzle({ puzzle, solved, onSubmit, onHint, loading }: Props) {
  ...
  function backspace() {
    ...
    const numIdx = data!.numbers.findIndex((n, i) =>
      String(n) === last && usedNumbers.includes(i)
    );
    if (numIdx !== -1) {
      setUsedNumbers((prev) => {
        const copy = [...prev];
        ...
        <>
          ...
          >
            <span style={{ color: tokens.length === 0 ? "var(--muted)" : undefined }}>
              {tokens.length > 0 ? tokens.join(" ") : "Click tiles to build your expression…"}
            </span>
            {showTotal && result !== null && (
              <span data-testid="expression-result" style={{ color: "var(--teal)", fontWeight: "bold", whiteSpace: "nowrap" }}>
                = {result}
              </span>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <button className="action-btn" onClick={backspace} disabled={tokens.length === 0} style={{ flex: 1 }}>
              <span className="gt">&gt;</span>Backspace
            </button>
            <button className="action-btn" onClick={clear} disabled={tokens.length === 0} style={{ flex: 1 }}>
