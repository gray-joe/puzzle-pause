...
export default function WordsearchBuilder({ question, answer, onChange }: Props) {
  ...
  function handleCellInput(e: React.FormEvent<HTMLInputElement>, r: number, c: number) {
    const val = (e.currentTarget.value).replace(/[^a-zA-Z]/g, "").slice(-1).toUpperCase();
    if (val) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16, marginBottom: 12, flexWrap: "wrap", alignItems: "center" }}>
        <label style={{ color: "var(--muted)", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input
            ...
            onChange={(e) => handleResizeRows(Math.min(15, Math.max(5, Number(e.target.value))))}
            style={{ width: 56 }}
          />
        </label>
        <label style={{ color: "var(--muted)", display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input
            ...
            onChange={(e) => handleResizeCols(Math.min(15, Math.max(5, Number(e.target.value))))}
            style={{ width: 56 }}
          />
        </label>
        <button type="button" className="action-btn" onClick={handleFillRandom} style={{ padding: "4px 10px" }}>
          Fill empty randomly
        </button>
        <button type="button" className="action-btn" onClick={handleClear} style={{ padding: "4px 10px" }}>
          Clear grid
        </button>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
        <div style={{ color: "var(--muted)", fontSize: "0.9em" }}>Words to find <span style={{ opacity: 0.6 }}>(stored as answer — not shown to players)</span></div>
        {words.map((word, index) => {
          ...
          return (
            <div key={index} style={{ display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {wordValid && (
                <span style={{ color: "var(--teal)", fontSize: "0.9em" }}>found in grid</span>
              )}
              {wordMissing && (
                <span style={{ color: "var(--red, #e05)", fontSize: "0.9em" }}>not found in grid</span>
              )}
            </div>
          );
        })}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        <label style={{ color: "var(--muted)", fontSize: "0.9em" }}>
          Theme / prompt <span style={{ opacity: 0.6 }}>(shown to players alongside the word count)</span>
        </label>
        <input
          type="text"
          value={theme}
