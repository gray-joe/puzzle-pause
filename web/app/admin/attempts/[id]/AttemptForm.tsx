...
interface Props { attempt: AdminAttempt }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Solved</label>
        <select value={form.solved ? "1" : "0"} onChange={(e) => set("solved", e.target.value === "1")}>
      ...
      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Score</label>
        <input type="number" value={form.score} onChange={(e) => set("score", e.target.value)} placeholder="—" />
      </div>

      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Wrong guesses</label>
        <input type="number" min={0} value={form.incorrect_guesses} onChange={(e) => set("incorrect_guesses", e.target.value)} />
      </div>

      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Hint used</label>
        <select value={form.hint_used ? "1" : "0"} onChange={(e) => set("hint_used", e.target.value === "1")}>
          <option value="0">no</option>
          <option value="1">yes</option>
        </select>
      </div>
      ...
      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Opened at</label>
        <input type="datetime-local" value={form.opened_at} onChange={(e) => set("opened_at", e.target.value)} />
      </div>

      <div style={fieldStyle}>
        <label style={labelStyle}><span className="gt">&gt;</span> Completed at</label>
      ...
      <button type="submit" className="action-btn" disabled={loading} style={{ marginTop: 8 }}>
        <span className="gt">&gt;</span>{loading ? "Saving..." : "Save Changes"}
      </button>
    </form>
  );
}
